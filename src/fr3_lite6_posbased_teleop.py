...
from utils.math_tools import calculate_delta_position, child_to_parent, parent_to_child, quantize_to_resolution, unwrap_angle
...
class MainLoop(RTLoop):
    ...
    def setup(self):
        ...
        fr3_home_pos = config["fr3_home_pos_table_mid"]
        ...
        self.write_fr3_position.open_fr3_gripper_async(width=0.05)
        print("Homing Arms")
        time.sleep(6)
        ...
        lite6_arm_pos_reading = self.read_lite6_position_loop.get_position_reading()
        delta6_pose_reading = self.delta6_lite6.get_FK_result()
        lite6_target = self.calculate_arm_target_pose(self.target_force, lite6_arm_pos_reading, delta6_pose_reading)
        self.last_lite6_target = lite6_target

        self.fr3_ee_trans = [0,0,0,0,0,0]
    ...
    def loop(self):
        ...
        lite6_target = self.calculate_arm_target_pose(self.target_force, lite6_pos_reading, delta6_pose_reading)
        
        lite6_target[3] = unwrap_angle(lite6_target[3], self.last_lite6_target[3])
        lite6_target[4] = unwrap_angle(lite6_target[4], self.last_lite6_target[4])
        lite6_target[5] = unwrap_angle(lite6_target[5], self.last_lite6_target[5])
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_w:
                        ...
                        self.write_fr3_position.close_fr3_gripper_async(width=0.015)
